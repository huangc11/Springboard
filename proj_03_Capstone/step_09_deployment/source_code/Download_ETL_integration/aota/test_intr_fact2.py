...
class intgrFightProcessor:
    ...
    def generate_fact_flight(self):
        ...
        ut_log.log_proc_metric(ut_log.get_log_id(), intgrFightProcessor.c_proc_name, df_fact_flt_repart, 'write', wf_path)
        ut_log.log_info("fact_flight created.")

        ut_log.log_proc_end(intgrFightProcessor.c_proc_name)


    def run(self):


        ut_log.log_proc_start(intgrFightProcessor.c_proc_name)

        self.read_data()
        self.prc_stg_dim_table()
        self.generate_fact_flight()

        ut_log.log_proc_start(intgrFightProcessor.c_proc_name)


factprocessor =intgrFightProcessor('1997')
